...
static const char *PG_default_params(void) {
...
static void set_PG_params(PG_params_t *PG_params, PG_init_params_t *init_params, uint64_t cache_size) {
  ...
  PG_params->cur_metadata_size += (size_t)HASH_SIZE * sizeof(Cluster*); // e.g. 4096 * 8 = 32 KB

  PG_params->cur_metadata_size += (size_t)MAX_FREQ * sizeof(Cluster*);  // e.g. 65536 * 8 = 512 KB
  ...
  PG_params->cur_metadata_size += md_init_size;
  ...
  PG_params->hash_1000  = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, NULL);
  PG_params->hash_10000  = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, NULL);
  PG_params->hash_100000  = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, NULL);
  PG_params->hash_1000000  = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, NULL);
...
prefetcher_t *create_PGCluster_prefetcher(const char *init_params, uint64_t cache_size) {
  ...
  if (init_params != NULL) {
    ...
    check_params_pg((PG_init_params));
  }
