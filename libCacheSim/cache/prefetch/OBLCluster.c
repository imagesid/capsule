...
static const char *OBL_default_params(void) { return "block-size=512, sequential-confidence-k=4, max_metadata_size=0.1"; }
...
static void set_OBL_params(OBL_params_t *OBL_params, OBL_init_params_t *init_params, uint64_t cache_size) {
  ...
  OBL_params->cur_metadata_size = 0;
//   OBL_params->max_metadata_size = (uint64_t)(init_params->block_size * cache_size * init_params->max_metadata_size);
  OBL_params->max_metadata_size = (uint64_t)(init_params->block_size * cache_size * 0.1);
  // Add global/static metadata size:
  OBL_params->cur_metadata_size += (size_t)HASH_SIZE * sizeof(Cluster*); // e.g. 4096 * 8 = 32 KB

  OBL_params->cur_metadata_size += (size_t)MAX_FREQ * sizeof(Cluster*);  // e.g. 65536 * 8 = 512 KB
  ...
  OBL_params->cur_metadata_size += md_init_size;
  
  OBL_params->ts = 0;
  OBL_params->cache_size_map =
      g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, NULL);

  OBL_params->hash_1000  = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, NULL);
  OBL_params->hash_10000  = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, NULL);
  OBL_params->hash_100000  = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, NULL);
  OBL_params->hash_1000000  = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, NULL);
...
static void OBL_handle_find(cache_t *cache, const request_t *req, bool hit) {
  ...
  g_hash_table_insert(OBL_params->cache_size_map,
...
static void OBL_prefetch(cache_t *cache, const request_t *req) {
  ...
  if (OBL_params->do_prefetch) {
    ...
    new_req->obj_size = GPOINTER_TO_INT(g_hash_table_lookup(
          OBL_params->cache_size_map, GINT_TO_POINTER(new_req->obj_id)));
...
static void free_OBL_prefetcher(prefetcher_t *prefetcher) {
  ...
  g_hash_table_destroy(OBL_params->cache_size_map);
...
prefetcher_t *create_OBLCluster_prefetcher(const char *init_params, uint64_t cache_size) {
  init(cache_size);
