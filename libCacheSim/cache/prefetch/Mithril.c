  ...
  // for logging
    if(total_prefetched > 0){ 
      ...
      snprintf(filepath2, sizeof(filepath2), "/dev/shm/prefetched%d.txt", cache->log);
            ...
            // FILE *fp = fopen("/dev/shm/prefetched4.txt", "w");
            if (fp != NULL) {
                fprintf(fp, "%d\n", prefetch_cumul);
                fflush(fp);
                fclose(fp);
                // last_cumul = prefetch_cumul;
...
static inline void _Mithril_record_entry(cache_t *cache, const request_t *req) {
  ...
  if (Mithril_params->min_support == 1) {
    ...
    if (index == 0) {
       ...
       **/
      // printf("Mithril_params->max_support %d\n", Mithril_params->max_support);
      if (index < 0) {
        /* in mining table */
        gint64 *row_in_mtable = GET_ROW_IN_MTABLE(Mithril_params, -index - 1);
