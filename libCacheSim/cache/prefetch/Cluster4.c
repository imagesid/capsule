//
//  a Mithril module that supports different obj size + CAPSULE
//
//
//  Cluster4.c
//  libCacheSim
...
static int scheme = Mithril;
static int conventional = -1;
...
static const char *Mithril_default_params(void) {
...
static size_t additional_size=0;
...
static void set_Mithril_params(Mithril_params_t *Mithril_params,
  ...
  Mithril_params->max_metadata_size =
      (gint64)(init_params->block_size * cache_size *
              ...
              //  printf("Mithril_params->max_metadata_size %d\n",Mithril_params->max_metadata_size);
  ...
  additional_size += (size_t)HASH_SIZE * sizeof(Cluster*);
  additional_size += (size_t)MAX_FREQ * sizeof(Cluster*);

  // Mithril_params->cur_metadata_size += (size_t)HASH_SIZE * sizeof(Cluster*); // e.g. 4096 * 8 = 32 KB

  // Mithril_params->cur_metadata_size += (size_t)MAX_FREQ * sizeof(Cluster*);  // e.g. 65536 * 8 = 512 KB
  ...
  additional_size +=md_init_size;
  Mithril_params->cur_metadata_size += additional_size; 
  // Mithril_params->cur_metadata_size += md_init_size; 
  ...
  for (int i = 0; i < MAX_OBJID; i++) {
      t_1000[i] = 0;
  }
  Mithril_params->hash_1000  = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, NULL);
  Mithril_params->hash_10000  = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, NULL);
  Mithril_params->hash_100000  = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, NULL);
  Mithril_params->hash_1000000  = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, NULL);
...
static void Mithril_handle_find(cache_t *cache, const request_t *req,
  ...
  if(conventional > -1){
    if(!hit){
       conventional_cluster(cache, req, scheme, conventional);
...
static prefetcher_t *clone_Mithril_prefetcher(prefetcher_t *prefetcher,
                                       uint64_t cache_size) {
  return create_Mithril_prefetcher(prefetcher->init_params, cache_size);
}

prefetcher_t *create_Cluster4_prefetcher(const char *init_params,
                                        uint64_t cache_size, int conv) {
  init(cache_size);
  conventional = conv;
  ...
  if (init_params != NULL) {
    ...
    check_params_mithril((Mithril_init_params));
  }
...
static gint mining_table_entry_cmp(gconstpointer a, gconstpointer b) {
  return (gint)GET_NTH_TS(a, 1) - (gint)GET_NTH_TS(b, 1);
}

/* in debug */
static void print_one_line(gpointer key, gpointer value, gpointer user_data) {
...
/* in debug */
static void print_prefetch_table(Mithril_params_t *Mithril_params) {
...
static bool consider_metasize= true;
static gpointer keyx=0;
static gpointer key64=0;
...
static void _Mithril_add_to_prefetch_table(cache_t *cache, gpointer gp1,
  ...
  gint prefetch_table_index = GPOINTER_TO_INT64(
  ...
  if (prefetch_table_index) {
...
#ifdef SANITY_CHECK
      if (Mithril_params->ptable_array[dim1][dim2] != GPOINTER_TO_INT64(gp1)) {
        // fprintf(stderr, "ERROR prefetch table pos wrong %d %ld, dim %d %d\n",
        //         GPOINTER_TO_INT64(gp1),
    ...
    if (insert) {
      ...
      Mithril_params->ptable_array[dim1][dim2 + i] = GPOINTER_TO_INT64(gp2);
    }
  } else {
    ...
    if (Mithril_params->ptable_is_full) {
      ...
      g_hash_table_remove(
          Mithril_params->prefetch_hashtable,
          GINT64_TO_POINTER(Mithril_params->ptable_array[dim1][dim2]));
    ...
    Mithril_params->ptable_array[dim1][dim2 + 1] = GPOINTER_TO_INT64(gp2);
    Mithril_params->ptable_array[dim1][dim2] = GPOINTER_TO_INT64(gp1);
    ...
    if (g_hash_table_contains(Mithril_params->prefetch_hashtable, gp1)) {
      ...
      //        GPOINTER_TO_INT64(gp), prefetch_table_index);
    }
#endif

    g_hash_table_insert(Mithril_params->prefetch_hashtable, gp1,
                        GINT64_TO_POINTER(Mithril_params->ptable_cur_row));
    ...
    if ((Mithril_params->ptable_cur_row + 1) % PREFETCH_TABLE_SHARD_SIZE == 0) {
      ...
      if (Mithril_params->cur_metadata_size +
        ...
        // if(consider_metasize) {
          ...
          cache->cache_size -= required_meta_data_size;
          // printf("cache_size %d \n",cache->cache_size);
          // consider_metasize= false;
        //   // delay the eviction
        // }
      } else {
