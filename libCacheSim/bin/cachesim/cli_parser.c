...
enum argp_option_short {
  ...
  OPTION_FIXED_PARAMS = 0x110,
  OPTION_ADAPTIVE_PARAMS =  0x111,
  OPTION_NEIGHBOR_PARAMS =  0x112,
  OPTION_IO_PARAMS =  0x113,
  OPTION_NOCACHE_PARAMS =  0x114,
  OPTION_MAX_PREFETCH_PARAMS =  0x115,
  OPTION_LOG_PARAMS =  0x116,
  OPTION_MAX_METADATA =  0x117,
  OPTION_DEBUG =  0x999,
};
...
static struct argp_option options[] = {
    ...
    {"conventional", OPTION_CONV_ALGO, "", 0,
     "Conventional algorithm: DBSCAN/KMEANS", 4},
    ...
    //tambahan
    {"max-metadata", OPTION_MAX_METADATA, "0.1", 0,"max metadata: 0-1 default 0.1", 4},
    {"fixed", OPTION_FIXED_PARAMS, "0", 0,"fixed epsilon: 0 or 512 or 512k default 0", 4},
    {"adaptive", OPTION_ADAPTIVE_PARAMS, "true", 0,"adaptive epsilon: true/false default true", 4},
    {"neighbor", OPTION_NEIGHBOR_PARAMS, "0", 0,"neighbor: true/false default false", 4},
    {"io", OPTION_IO_PARAMS, "false", 0,"io: true/false default false", 4},
    {"nocache", OPTION_NOCACHE_PARAMS, "false", 0,"io: true/false default false", 4},
    {"max-prefetch", OPTION_MAX_PREFETCH_PARAMS, "200000", 0,"Max prefetch default 100/trigger/bucket", 10},
    {"log", OPTION_LOG_PARAMS, "0", 0,"ID Log default 0", 4},
    {"debug", OPTION_DEBUG, "0", 0,"debug: true/false default false", 4},
...
static error_t parse_opt(int key, char *arg, struct argp_state *state) {
  ...
  switch (key) {
    ...
    case OPTION_PREFETCH_ALGO:
      ...
      break;
    case OPTION_CONV_ALGO:
      arguments->conv_algo = arg;
      break;
    //tambahan
    case OPTION_MAX_METADATA:
      arguments->max_metadata = atof(arg);
      break;
    case OPTION_FIXED_PARAMS:
      arguments->fixed = atoi(arg);
      break;
    case OPTION_ADAPTIVE_PARAMS:
      arguments->adaptive = is_true(arg) ? true : false;
      break;
    case OPTION_NEIGHBOR_PARAMS:
      arguments->neighbor = is_true(arg) ? true : false;
      break;
    case OPTION_IO_PARAMS:
      arguments->io = is_true(arg) ? true : false;
      break;
    case OPTION_NOCACHE_PARAMS:
      arguments->nocache = is_true(arg) ? true : false;
      break;
    case OPTION_MAX_PREFETCH_PARAMS:
      arguments->max_prefetch = atoi(arg);   
      break;
    case OPTION_LOG_PARAMS:
      arguments->log = atoi(arg);   
      break;
    case OPTION_DEBUG:
      arguments->debug = is_true(arg) ? true : false;
      break;
...
static void init_arg(struct arguments *args) {
  ...
  // tambahan
  args->max_metadata = 0.1;
  args->fixed = 0;
  args->adaptive = true;
  args->neighbor = false;
  args->max_prefetch = 100000;
  args->io = false;
  // end tambahan
  ...
  for (int i = 0; i < args->n_eviction_algo; i++) {
    for (int j = 0; j < args->n_cache_size; j++) {
      ...
      if (args->prefetch_algo != NULL) {
        // printf("Max Metadatayyy: %f\n", args->max_metadata);

        if (args->conv_algo != NULL) {
          args->caches[idx]->prefetcher = create_prefetcher(
            args->prefetch_algo, args->prefetch_params, args->cache_sizes[j], args->conv_algo,args->max_metadata);
        }else{
          args->caches[idx]->prefetcher = create_prefetcher(
            args->prefetch_algo, args->prefetch_params, args->cache_sizes[j], "",args->max_metadata);
        }
...
#define OUTPUT_STR_LEN 1024
  ...
  if (args->conv_algo != NULL) {
    n += snprintf(output_str + n, OUTPUT_STR_LEN - n - 1, ", conventional: %s",
                  args->conv_algo);
  }
