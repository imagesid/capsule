...
# === Input data ===
df = pd.read_csv("logs/result-cp.csv")

df["Filename"] = df["Filename"].str.replace(
    r"\.oracleGeneral\.bin$", "", regex=True
)
...
ax.legend(handles=legend_elements, fontsize=5, ncol=4, loc='upper center',
          bbox_to_anchor=(0.5, -0.22), frameon=False)

# === Style and axis labels ===
ax.set_xlabel("Workload", fontsize=6)
ax.set_ylabel("Hit Rate (%)", fontsize=6)
ax.set_xticks(x)
